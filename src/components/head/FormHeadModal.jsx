...
export default function ModalCreate({ open, setOpen }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await createUser("/admin/truongbomon/create-tbm", newHead);
      setOpen(false);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true);
        const [subjectsData] = await Promise.all([getSubjects()]);
        const subjectOptions = subjectsData.data.subjects.map((subject) => ({
